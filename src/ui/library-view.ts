...
export class MediaLibraryView extends ItemView {
  ...
  async render(): Promise<void> {
    ...
    if (dashboard) {
      dashboard.createDiv( { cls: 'pml-dashboard-stat', text: `${analytics.total} items` });
      dashboard.createDiv( { cls: 'pml-dashboard-stat', text: `${analytics.favorites} favorites` });
      dashboard.createDiv( { cls: 'pml-dashboard-stat', text: `${analytics.completed} completed` });
      dashboard.createDiv( { cls: 'pml-dashboard-stat', text: analytics.averageRating !== undefined ? `★ ${analytics.averageRating.toFixed(1)} avg` : 'No ratings' });
    }
    ...
    if (!items.length) {
      contentEl.createDiv( { cls: 'pml-empty', text: 'No media entries yet. Add a URL to create your first entry.' });
      return;
    }
    ...
    const renderFiltered = (): void => {
      ...
      if (!filtered.length) {
        container.createDiv( { cls: 'pml-empty', text: 'No media matches the current filters.' });
      }
    };
